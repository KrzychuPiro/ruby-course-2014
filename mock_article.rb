...
class MockArticle < Article
  ...
  def generate_title
    title = ''
    Random.rand(1..5).times { title += "#{@@WORD_LIST[Random.rand(@@LENGTH-1)]} " }
    title.capitalize.rstrip
  end
  
  def generate_sentence
    sentence = ''
    Random.rand(5..10).times { sentence += "#{@@WORD_LIST[Random.rand(@@LENGTH-1)]} " }
    sentence.capitalize.rstrip + '.'
  end
